...
#     network_df['log_messages'] = network_df['messages'].apply(lambda x: np.log(x))
#     return network_df


def create_network_group_df(df, slice=50):
    df['message_lengths'] = df['messages'].apply(lambda x: len(x))
    df = df.sort_values(['message_lengths'], ascending=False)
    df = df[:slice]  # plot top 50 groups
    name, connections, messages = [], [], []
    for index, row in df.iterrows():
        message_counter = Counter([message['sender_name'] for message in row['messages']])
        participants = len(row['participants'])
        if participants > 2:  # no connections to for self-messages
            for i in range(len(row['participants'])):
...
def plot_group_network(df, seed=None, annotate=True):
    plt.figure()
    ...
    weights = [np.log(graph.degree(node, weight='messages') / graph.degree(node)) * 200 for node in graph.nodes() if
               node in df['connections'].unique()]
    ...
    nx.draw_networkx_nodes(graph, layout, nodelist=people, node_size=100, node_color='#AAAAAA')  # grey
...
if __name__ == '__main__':
    ...
    # Network plot
    network_group_df = create_network_group_df(messenger_data, slice=60)
    plot_group_network(network_group_df, seed=88, annotate=annotate_plots)
